...
void MyFrame::OnHello(wxCommandEvent& event)
{
    ...
    std::string data = R"({ "username": "adminsx", "password": "Adminsx@123" })";
    httplib::Client cli("http://192.168.3.111:8090");

    httplib::Headers headers = {
        { "content-type", "application/json" }
    };

    auto res = cli.Post("/login",headers, data, "application/json");
    int status = res->status;
    std::string stlstring = res->body;
